...
async def add_coupon(code, discount_percent, max_uses, service_id=None):
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute(
            "INSERT INTO coupons (code, discount_percent, max_uses, service_id) VALUES (?, ?, ?, ?)",
            (code.upper(), discount_percent, max_uses, service_id),
        )
        await db.commit()
        return cursor.lastrowid
