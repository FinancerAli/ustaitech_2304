...
async def deliver_form(call: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    if not service or not service.get("form_instruction"):
        await call.answer("Forma qo'llanmasi topilmadi.", show_alert=True)
        return
    ...
    await call.message.reply(
        ...
        "📥 Endi mijozning javobi kutilmoqda."
    )
...
async def adm_form_fulfilled(call: CallbackQuery, bot: Bot):
    ...
    await call.message.reply(f"✅ Buyurtma #{order_id} yakunlandi va mijozga xabar yuborildi.")
...
async def adm_add_delivery_mode(message: Message, state: FSMContext):
    if message.text in ADMIN_CANCEL_TEXTS:
        ...
        await message.answer("Bekor qilindi.", reply_markup=admin_menu())
        return
    delivery_mode = (message.text or "").strip()
    if not _is_valid_delivery_mode(delivery_mode):
        await message.answer("❌ Quyidagilardan birini kiriting: manual, key_auto, form_required, referral_activation")
        return
    await state.update_data(
...
async def adm_edit_supports_stars(message: Message, state: FSMContext):
    ...
    await message.answer(
        "▫️ Yangi delivery mode tanlang:",
        reply_markup=delivery_mode_keyboard(edit_mode=True),
    )


async def _finish_edit_service(message: Message, state: FSMContext):
    data = await state.get_data()
    await db.update_service(
        data["edit_id"], data["new_name"], data.get("new_desc_uz", ""),
        data["new_price"], description_ru=data.get("new_desc_ru", ""),
        stars_price=data.get("new_stars_price", 0),
        supports_stars=data.get("new_supports_stars", 0),
        delivery_mode=data.get("new_delivery_mode", "manual"),
        required_referrals=data.get("new_required_referrals", 0),
        requires_email=data.get("new_requires_email", 0),
        referral_deadline_days=data.get("new_referral_deadline_days", 0),
    )
    await state.clear()
    await message.answer(
...
async def adm_edit_delivery_mode(message: Message, state: FSMContext):
    ...
    if message.text != "-":
        ...
        delivery_mode = service.get("delivery_mode", "manual")
    await state.update_data(
        new_delivery_mode=delivery_mode,
        new_required_referrals=int(service.get("required_referrals") or 0),
        new_requires_email=int(service.get("requires_email") or 0),
        new_referral_deadline_days=int(service.get("referral_deadline_days") or 0),
    )
    if delivery_mode == "referral_activation":
...
@router.message(EditServiceState.required_referrals)
async def adm_edit_required_referrals(message: Message, state: FSMContext):
    if message.text in ADMIN_CANCEL_TEXTS:
        await state.clear()
        await message.answer("Bekor qilindi.", reply_markup=admin_menu())
        return
...
async def adm_edit_requires_email(message: Message, state: FSMContext):
    if message.text in ADMIN_CANCEL_TEXTS:
        await state.clear()
        await message.answer("Bekor qilindi.", reply_markup=admin_menu())
        return
    ...
    if message.text != "-":
        ...
        value = int(service.get("requires_email") or 0)
    await state.update_data(new_requires_email=value)
    await state.set_state(EditServiceState.referral_deadline_days)
    await message.answer("ℹ️ Deadline days ('-' = o'zgartirmaslik, 0/7/14/30):", reply_markup=cancel_keyboard())


@router.message(EditServiceState.referral_deadline_days)
async def adm_edit_referral_deadline_days(message: Message, state: FSMContext):
    if message.text in ADMIN_CANCEL_TEXTS:
        await state.clear()
        await message.answer("Bekor qilindi.", reply_markup=admin_menu())
        return
    ...
    if message.text != "-":
        ...
        value = int(service.get("referral_deadline_days") or 0)
    await state.update_data(new_referral_deadline_days=value)
    await _finish_edit_service(message, state)
...
async def adm_ref_camp_reward(message: Message, state: FSMContext):
    ...
    await message.answer("Kampaniya uchun tugash muddati (deadline) kiriting (masalan: 1-Maygacha, 2025-yilgacha, yoki Yoq deb yozing):", reply_markup=cancel_keyboard())
...
async def adm_ref_camp_deadline(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    if not message.text:
        await message.answer("Iltimos, matn kiriting (masalan: 1-Maygacha yoki Yoq):")
        return
    if message.text in ("❌ Bekor qilish", "Bekor qilish", "Отмена"):
        await state.clear()
        await message.answer("Bekor qilindi.", reply_markup=admin_menu())
    ...
    skip_values = ("yo'q", "yoq", "yo'q", "net", "no", "skip", "/skip", "-", "0")
    deadline = None if message.text.strip().lower() in skip_values else message.text.strip()
    data = await state.get_data()
    reward_type = data.get("camp_reward_type", "bonus")
    ...
    try:
        ...
        logging.error(f"Campaign create error: {e}")
        await message.answer(f"❌ Xatolik yuz berdi: {e}\nQayta urinib ko'ring.", reply_markup=cancel_keyboard())
