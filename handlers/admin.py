...
async def deliver_standard(call: CallbackQuery, bot: Bot, state: FSMContext):
    print(f"DEBUG_ROUTING: adm_deliver_std triggered with data={call.data}", flush=True)
    await call.answer(f"DEBUG: std hit! data={call.data}", show_alert=True)
    order_id = int(call.data.split(":")[1])
    order = await db.get_order(order_id)
    if not order:
        await call.answer("Buyurtma topilmadi.", show_alert=True)
...
async def deliver_custom_start(call: CallbackQuery, state: FSMContext):
    print(f"DEBUG_ROUTING: adm_deliver_custom triggered with data={call.data}", flush=True)
    await call.answer(f"DEBUG: custom hit! data={call.data}", show_alert=True)
    if not is_admin(call.from_user.id):
        return
    order_id = int(call.data.split(":")[1])
    order = await db.get_order(order_id)
...
async def deliver_skip(call: CallbackQuery, bot: Bot):
    print(f"DEBUG_ROUTING: adm_deliver_skip triggered with data={call.data}", flush=True)
    await call.answer(f"DEBUG: skip hit! data={call.data}", show_alert=True)
    if not is_admin(call.from_user.id):
        return
    order_id = int(call.data.split(":")[1])
    order = await db.get_order(order_id)
...
async def deliver_form(call: CallbackQuery, bot: Bot, state: FSMContext):
    print(f"DEBUG_ROUTING: adm_deliver_form triggered with data={call.data}", flush=True)
    await call.answer(f"DEBUG: form hit! data={call.data}", show_alert=True)
    if not is_admin(call.from_user.id):
        return
    order_id = int(call.data.split(":")[1])
    order = await db.get_order(order_id)
...
async def adm_service_detail(call: CallbackQuery):
    ...
    await call.message.edit_text(text, reply_markup=service_admin_detail(service_id, s["active"], bool(s["delivery_content"]), bool(s.get("form_instruction"))), parse_mode="HTML")
...
async def add_coupon_max(message: Message, state: FSMContext):
    ...
    try:
        coupon_id = await db.add_coupon(data["code"], data["discount"], int(message.text), service_id=service_id)
    ...
    await message.answer(
        ...
        f"🎯 Amal qilish doirasi: <b>{scope_text}</b>",
        reply_markup=admin_menu(),
        parse_mode="HTML",
    )
