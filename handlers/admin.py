...
from keyboards.admin_kb import (
    ...
    form_fulfilled_keyboard, referral_order_list_keyboard, referral_order_detail_keyboard,
    ref_campaigns_admin_keyboard, ref_campaign_admin_detail_keyboard,
    ref_campaign_participant_keyboard, delivery_mode_keyboard,
    service_stock_type_keyboard, service_payment_type_keyboard, service_delivery_type_keyboard,
    bonus_monitoring_keyboard
)
...
def _service_mode_label(mode: str) -> str:
    ...
    return labels.get(mode or "manual", mode or "manual")


def _referral_summary_text(service: dict) -> str:
    if not service or service.get("delivery_mode") != "referral_activation":
        return ""
    requires_email = "Ha" if int(service.get("requires_email") or 0) else "Yo'q"
...
def _is_valid_delivery_mode(value: str) -> bool:
    return value in {"manual", "key_auto", "form_required", "referral_activation"}


async def _get_referral_orders_by_filter(filter_name: str):
    if filter_name == "ready":
        return await db.get_referral_orders(activation_status="ready")
    if filter_name == "expired":
        return await db.get_referral_orders(referral_status="expired")
    if filter_name == "activated":
        return await db.get_referral_orders(activation_status="activated")
    return await db.get_referral_orders(activation_status="pending")


async def _build_referral_order_detail(order_id: int) -> str:
    ...
    history = await db.get_referral_invites(order_id)
    history_text = ""
    if history:
        history_lines = []
        for item in history[:10]:
            history_lines.append(
                f"  • <code>{item['invited_user_id']}</code> | {item['status']} | {str(item['created_at'])[:16]}"
            )
    ...
    ref_emoji = {"awaiting_referrals": "⏳", "awaiting_email": "📧", "target_met": "✅",
                 "expired": "⏰", "cancelled": "❌"}.get(ref_status, "❓")
    act_emoji = {"pending": "⏳", "ready": "🟢", "activated": "✅", "failed": "❌"}.get(act_status, "❓")

    # Deadline warning
    deadline_raw = order.get("referral_deadline_at")
    if deadline_raw:
        deadline_str = str(deadline_raw)[:16]
        deadline_text = f"⏳ {deadline_str}"
        if ref_status == "expired":
            deadline_text = f"⚠️ <b>MUDDATI O'TGAN</b> ({deadline_str})"
    else:
        deadline_text = "belgilanmagan"

    username = f"@{user['username']}" if user and user.get("username") else "nomalum"
    fulfillment = order.get("fulfillment_status", "pending")
    return (
        ...
        f"{bar}\n\n"
        f"{ref_emoji} Referral: <b>{ref_status}</b>\n"
        f"{act_emoji} Aktivatsiya: <b>{act_status}</b>\n"
        f"📦 Yetkazish: <b>{fulfillment}</b>\n"
        f"📅 Deadline: {deadline_text}\n"
        f"🕐 Ready at: {order.get('activation_ready_at') or '-'}\n"
        f"✅ Activated at: {order.get('activated_at') or '-'}"
        f"{history_text}"
    )


async def _ref_detail_kb(order_id: int, current_filter: str = "pending"):
    """Build referral_order_detail_keyboard with proper status params from DB."""
    order = await db.get_order(order_id)
    act = order.get("activation_status", "pending") if order else "pending"
    ref = order.get("referral_status", "awaiting_referrals") if order else "awaiting_referrals"
    return referral_order_detail_keyboard(order_id, current_filter, activation_status=act, referral_status=ref)
...
class DeliveryKeyGuideState(StatesGroup):
    key = State()
...
async def referral_activations(message: Message):
    if not is_admin(message.from_user.id):
        return
    orders = await _get_referral_orders_by_filter("pending")
    if not orders:
        await message.answer("🎯 <b>Referral aktivatsiyalar</b> (Buyurtmalar)\n\nHozircha pending referral buyurtmalar yo'q.\n\n<i>Eslatma: Kampaniya aktivatsiyalarini ko'rish uchun \"🏆 Referral kampaniyalar\" menyusidan foydalaning!</i>", parse_mode="HTML")
        return
    await message.answer(
        "🎯 <b>Referral aktivatsiyalar</b> (Buyurtmalar)\n\n<i>Eslatma: Kampaniya aktivatsiyalarini ko'rish uchun \"🏆 Referral kampaniyalar\" menyusidan foydalaning!</i>",
        reply_markup=referral_order_list_keyboard(orders, "pending"),
        parse_mode="HTML",
    )


@router.callback_query(F.data.startswith("adm_ref_list:"))
async def referral_activations_list(call: CallbackQuery):
    ...
    await call.message.edit_text(
        text,
        reply_markup=referral_order_list_keyboard(orders, filter_name),
        parse_mode="HTML",
    )


@router.callback_query(F.data.startswith("adm_ref_view:"))
async def referral_activation_view(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return
    _, order_id_raw, current_filter = call.data.split(":")
    order_id = int(order_id_raw)
    await call.answer()
    text = await _build_referral_order_detail(order_id)
    await call.message.edit_text(
        text,
        reply_markup=await _ref_detail_kb(order_id, current_filter),
        parse_mode="HTML",
    )


@router.callback_query(F.data.startswith("adm_ref_deadline:"))
async def referral_activation_deadline(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return
    _, order_id_raw, days_raw, current_filter = call.data.split(":")
    order_id = int(order_id_raw)
    days = int(days_raw)
    deadline_at = db.build_referral_deadline(days)
    await db.set_referral_deadline(order_id, deadline_at)
    await call.answer("Deadline yangilandi.")
    text = await _build_referral_order_detail(order_id)
    await call.message.edit_text(
        text,
        reply_markup=await _ref_detail_kb(order_id, current_filter),
...
@router.callback_query(F.data.startswith("adm_ref_activate:"))
async def referral_activation_activate(call: CallbackQuery, bot: Bot):
    if not is_admin(call.from_user.id):
        return
    _, order_id_raw, current_filter = call.data.split(":")
    order_id = int(order_id_raw)
    ...
    if not order:
        ...
        return
    await db.mark_referral_activated(order_id)
    await call.answer("Aktivlashtirildi!")
    user = await db.get_user(order["user_id"])
    ...
    try:
        await bot.send_message(
            order["user_id"],
            (
                "✅ Xizmatingiz aktivlashtirildi!\n\nAdmin tez orada xizmatni yetkazadi."
                if user_lang != "ru" else
                "✅ Ваша услуга активирована!\n\nАдмин скоро доставит услугу."
            ),
        )
    except Exception:
        pass
    ...
    service = normalize_service(await db.get_service(order["service_id"]))
    has_preset = bool(service and service.get("delivery_content"))
    has_form = bool(service and service.get("form_instruction"))
    await call.message.edit_reply_markup(reply_markup=None)
    await call.message.reply(
        f"✅ Buyurtma #{order_id} aktivlashtirildi.\n\n📦 Endi yetkazish usulini tanlang:",
        reply_markup=delivery_choose_keyboard(order_id, has_preset, has_form),
    )


@router.callback_query(F.data.startswith("adm_ref_deliver:"))
async def referral_activation_deliver(call: CallbackQuery, bot: Bot):
    """Admin clicks 'Yetkazishni boshlash' from referral order detail."""
    if not is_admin(call.from_user.id):
        return
    _, order_id_raw, current_filter = call.data.split(":")
    order_id = int(order_id_raw)
    ...
    await call.message.edit_reply_markup(reply_markup=None)
    await call.message.reply(
        f"📦 Buyurtma #{order_id} uchun yetkazish usulini tanlang:",
        reply_markup=delivery_choose_keyboard(order_id, has_preset, has_form),
    )



@router.callback_query(F.data.startswith("adm_ref_cancel:"))
async def referral_activation_cancel(call: CallbackQuery, bot: Bot):
    if not is_admin(call.from_user.id):
        return
    _, order_id_raw, current_filter = call.data.split(":")
    order_id = int(order_id_raw)
    ...
    if not order:
        await call.answer("Buyurtma topilmadi.", show_alert=True)
        return
    await db.cancel_referral_order(order_id)
    await call.answer("Referral bekor qilindi.")
    try:
        await bot.send_message(
            order["user_id"],
            "❌ Referral aktivatsiya buyurtmangiz bekor qilindi. Savollar bo'lsa admin bilan bog'laning.",
        )
    except Exception:
        pass
    text = await _build_referral_order_detail(order_id)
...
async def confirm_order(call: CallbackQuery, bot: Bot):
    ...
    if not order:
        await call.answer("Buyurtma topilmadi!", show_alert=True)
        return
    await db.confirm_order_for_processing(order_id)
    ...
    if service and service.get("delivery_mode") == "referral_activation":
        detail_text = await _build_referral_order_detail(order_id)
        await call.message.reply(
            f"✅ Buyurtma #{order_id} tasdiqlandi.\n\n{detail_text}",
            reply_markup=await _ref_detail_kb(order_id, "pending"),
            parse_mode="HTML",
        )
        return
...
def _build_keyguide_message(key_text: str, delivery_content: str) -> str:
    safe_key = html.escape((key_text or "").strip())
    body = (delivery_content or "").strip()
    ...
    parts.extend([
        "",
        "ℹ️ Savol tug'ilsa, shu chat orqali bizga yozishingiz mumkin."
    ])

    return "\n".join(parts)
@router.callback_query(F.data.startswith("adm_deliver_keyguide:"))
async def deliver_keyguide_start(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        return
    ...
    if not service or not service["delivery_content"]:
    	await call.answer("Bu xizmat uchun qo'llanma shabloni topilmadi.", show_alert=True)
    	return

    delivery_content = service["delivery_content"].strip()

    await state.update_data(
        keyguide_order_id=order_id,
        keyguide_user_id=order["user_id"],
        keyguide_service_name=order["service_name"],
        keyguide_delivery_content=delivery_content,
    )
    await state.set_state(DeliveryKeyGuideState.key)
...
async def deliver_keyguide_send(message: Message, state: FSMContext, bot: Bot):
    ...
    if not key_text:
        await message.answer(
            "❌ Key bo'sh bo'lmasin. Qayta yuboring.",
            reply_markup=cancel_keyboard(),
        )
        return
    ...
    final_text = _build_keyguide_message(
        key_text=key_text,
        delivery_content=data.get("keyguide_delivery_content", ""),
    )
    ...
    try:
        ...
        await db.update_fulfillment_status(data["keyguide_order_id"], "delivered")
        await message.answer(
            "✅ Key + qo'llanma mijozga yuborildi!",
            reply_markup=admin_menu(),
        )

        await _send_review_request(
            bot,
            data["keyguide_order_id"],
            data["keyguide_user_id"],
            data["keyguide_service_name"],
        )
...
async def deliver_skip(call: CallbackQuery, bot: Bot):
    ...
    await db.update_fulfillment_status(order_id, "delivered")
    await call.message.edit_reply_markup(reply_markup=None)
    await call.answer("Botdan tashqari bajarildi.")
    await _send_review_request(bot, order_id, order["user_id"], order["service_name"])
    asyncio.create_task(_delayed_review_request(bot, order_id, order["user_id"], order["service_name"]))
    await call.message.reply(f"✅ Buyurtma #{order_id} botdan tashqari bajarildi deb yopildi.")
...
async def adm_user_search_start(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        return
    await call.answer()
...
async def adm_user_search_result(message: Message, state: FSMContext):
    ...
    if not query:
        await message.answer("❗ Bo'sh qidiruv. Qaytadan kiriting:")
        return
    ...
    for u in users:
        ...
        if query in uid_str or query in uname or query in fname or (query.startswith("@") and query[1:] in uname):
            results.append(u)
    ...
    for u in results:
        ...
        if len(label) > 50:
            label = label[:47] + "..."
        buttons.append([InlineKeyboardButton(text=label, callback_data=f"adm_user:{u['id']}")])
...
# BROADCAST (eski sodda broadcast o'chirildi, pastdagi to'liq variant ishlatiladi)
...
async def adm_add_name(message: Message, state: FSMContext):
    ...
    if not name or name == "-":
        ...
        return
    await state.update_data(name=name, description_uz="", description_ru="", image_file_id=None, delivery_content=None)
    await state.set_state(AddServiceState.price)
    await message.answer("💰 Narxni so'mda kiriting:\n\nMasalan: 48000", reply_markup=cancel_keyboard())
...
async def adm_add_price(message: Message, state: FSMContext):
    ...
    await message.answer("📦 Qoldiq turini tanlang:\n\n♾ Cheksiz — limit yo'q\n🔢 Aniq dona — son kiritiladi", reply_markup=service_stock_type_keyboard())
...
async def adm_add_stock(message: Message, state: FSMContext):
    ...
    if text == "♾ Cheksiz" or text.lower() in {"cheksiz", "unlimited", "-1", "99999"}:
        await state.update_data(stock=-1, stock_mode=None)
        await state.set_state(AddServiceState.supports_stars)
        await message.answer("💳 To'lov usulini tanlang:", reply_markup=service_payment_type_keyboard())
        return

    if text == "🔢 Aniq dona":
        await state.update_data(stock_mode="finite")
        await message.answer("🔢 Nechta mavjud? Faqat raqam kiriting:", reply_markup=cancel_keyboard())
        return

    if not text.isdigit():
        await message.answer("❗ Qoldiq uchun ♾ Cheksiz, 🔢 Aniq dona yoki raqam kiriting:", reply_markup=service_stock_type_keyboard())
        return

    await state.update_data(stock=int(text), stock_mode=None)
    await state.set_state(AddServiceState.supports_stars)
    await message.answer("💳 To'lov usulini tanlang:", reply_markup=service_payment_type_keyboard())
...
async def adm_add_stars_price(message: Message, state: FSMContext):
    ...
    if not message.text.isdigit():
        ...
        return
    await state.update_data(stars_price=int(message.text), supports_stars=1)
    await state.set_state(AddServiceState.delivery_mode)
    await message.answer("🚚 Xizmat qanday bajariladi?", reply_markup=service_delivery_type_keyboard())
...
async def adm_add_supports_stars(message: Message, state: FSMContext):
    ...
    text = (message.text or "").strip()
    if text in {"💳 Manual", "0"}:
        await state.update_data(supports_stars=0, stars_price=0)
        await state.set_state(AddServiceState.delivery_mode)
        await message.answer("🚚 Xizmat qanday bajariladi?", reply_markup=service_delivery_type_keyboard())
        return

    if text in {"⭐ Stars", "💳⭐ Ikkalasi", "1"}:
        await state.update_data(supports_stars=1)
        await state.set_state(AddServiceState.stars_price)
        await message.answer("⭐️ Telegram Stars narxini kiriting:\n\nMasalan: 192", reply_markup=cancel_keyboard())
        return

    await message.answer("❗ To'lov usulini tugmalardan tanlang:", reply_markup=service_payment_type_keyboard())


async def _finish_add_service(message: Message, state: FSMContext):
    data = await state.get_data()
    service_id = await db.add_service(
        ...
        supports_stars=data.get("supports_stars", 0),
    ...
    stock_text = "♾ Cheksiz" if stock_value == -1 else f"{stock_value} dona"
    stars_text = f"\n⭐ Stars: {data.get('stars_price', 0)} XTR" if data.get("supports_stars") else "\n⭐ Stars: o'chirilgan"
    await message.answer(
        f"✅ Xizmat qo'shildi #{service_id}: <b>{data['name']}</b>\n"
        f"💰 Narx: <b>{data['price']:,} so'm</b>\n"
        f"📦 Qoldiq: <b>{stock_text}</b>{stars_text}\n"
        f"🚚 Delivery: <code>{data.get('delivery_mode', 'manual')}</code>\n"
        f"{delivery_status}{referral_status}\n\n"
        "Kengaytirilgan sozlamalar uchun: 🛍 Xizmatlar → xizmatni tanlang → Tahrirlash / Forma / Yetkazish.",
        reply_markup=admin_menu(),
        parse_mode="HTML",
    )


@router.message(AddServiceState.delivery_mode)
async def adm_add_delivery_mode(message: Message, state: FSMContext):
    if message.text in ADMIN_CANCEL_TEXTS:
        await state.clear()
        await message.answer("Bekor qilindi.", reply_markup=admin_menu())
        return
    ...
    text = (message.text or "").strip()
    delivery_map = {
        "📋 Manual (qo'lda)": "manual",
        "⚡ Avtomatik (shablon)": "key_auto",
        "🧾 Forma to'ldirish": "form_required",
        "🎯 Referral aktivatsiya": "referral_activation",
        "manual": "manual",
        "key_auto": "key_auto",
        "form_required": "form_required",
        "referral_activation": "referral_activation",
    }
    delivery_mode = delivery_map.get(text)
    if not delivery_mode:
        await message.answer("❌ Tugmalardan birini tanlang:", reply_markup=service_delivery_type_keyboard())
        return
...
@router.message(AddServiceState.required_referrals)
async def adm_add_required_referrals(message: Message, state: FSMContext):
    if message.text in ADMIN_CANCEL_TEXTS:
        await state.clear()
        await message.answer("Bekor qilindi.", reply_markup=admin_menu())
        return
...
async def adm_add_requires_email(message: Message, state: FSMContext):
    if message.text in ADMIN_CANCEL_TEXTS:
        await state.clear()
        await message.answer("Bekor qilindi.", reply_markup=admin_menu())
        return
    ...
    await message.answer("ℹ️ Referral deadline days kiriting (0, 7, 14, 30):", reply_markup=cancel_keyboard())


@router.message(AddServiceState.referral_deadline_days)
async def adm_add_referral_deadline_days(message: Message, state: FSMContext):
    if message.text in ADMIN_CANCEL_TEXTS:
        await state.clear()
        await message.answer("Bekor qilindi.", reply_markup=admin_menu())
        return
    if message.text not in ["0", "7", "14", "30"]:
...
async def adm_edit_start(call: CallbackQuery, state: FSMContext):
    ...
    s = normalize_service(await db.get_service(service_id))
    if not s:
        await call.message.answer("Xizmat topilmadi.", reply_markup=admin_menu())
        return
    stars_text = f"{s.get('stars_price', 0)} XTR" if s.get("supports_stars") else "o'chirilgan"
    text = (
        f"✏️ <b>{s['name']}</b> tahrirlash\n\n"
        f"💰 Narx: {s['price']:,} so'm\n"
        f"⭐ Stars: {stars_text}\n"
        f"🚚 Mode: {s.get('delivery_mode', 'manual')}\n\n"
        f"Qaysi maydonni o'zgartirmoqchisiz?"
    )
    buttons = [
        [InlineKeyboardButton(text="✏️ Nom", callback_data=f"adm_ef:name:{service_id}"),
         InlineKeyboardButton(text="💰 Narx", callback_data=f"adm_ef:price:{service_id}")],
        [InlineKeyboardButton(text="📝 Tavsif (UZ)", callback_data=f"adm_ef:desc_uz:{service_id}"),
         InlineKeyboardButton(text="📝 Tavsif (RU)", callback_data=f"adm_ef:desc_ru:{service_id}")],
        [InlineKeyboardButton(text="⭐ Stars narx", callback_data=f"adm_ef:stars:{service_id}"),
         InlineKeyboardButton(text="🚚 Delivery mode", callback_data=f"adm_ef:delivery:{service_id}")],
        [InlineKeyboardButton(text="🔙 Orqaga", callback_data=f"adm_service:{service_id}")],
    ]
    await call.message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons), parse_mode="HTML")


@router.callback_query(F.data.startswith("adm_ef:"))
async def adm_edit_field_pick(call: CallbackQuery, state: FSMContext):
    _, field, sid = call.data.split(":")
    service_id = int(sid)
    await call.answer()
    await state.update_data(edit_id=service_id, edit_field=field)

    prompts = {
        "name": "✏️ Yangi nom kiriting:",
        "price": "💰 Yangi narxni kiriting (raqam):",
        "desc_uz": "📝 Yangi tavsif (O'zbekcha):",
        "desc_ru": "📝 Yangi tavsif (Ruscha):",
        "stars": "⭐ Yangi Stars narxi (0 = o'chirish):",
        "delivery": "🚚 Delivery mode tanlang:",
    }
    if field == "delivery":
        await state.set_state(EditServiceState.delivery_mode)
        await call.message.answer(prompts[field], reply_markup=delivery_mode_keyboard(edit_mode=True))
    else:
        await state.set_state(EditServiceState.name)
        await call.message.answer(prompts[field], reply_markup=cancel_keyboard())


@router.message(EditServiceState.name)
async def adm_edit_single_field(message: Message, state: FSMContext):
    if message.text in ADMIN_CANCEL_TEXTS:
        await state.clear()
        await message.answer("Bekor qilindi.", reply_markup=admin_menu())
        return
    data = await state.get_data()
    service_id = data["edit_id"]
    ...
    if field == "name":
        if not text:
            ...
            return
        await db.execute_sql(f"UPDATE services SET name = ? WHERE id = ?", (text, service_id))
    elif field == "price":
        if not text.isdigit():
            await message.answer("❗ Faqat raqam kiriting:")
            return
        await db.execute_sql(f"UPDATE services SET price = ? WHERE id = ?", (int(text), service_id))
    elif field == "desc_uz":
        await db.execute_sql(f"UPDATE services SET description_uz = ?, description = ? WHERE id = ?", (text, text, service_id))
    elif field == "desc_ru":
        await db.execute_sql(f"UPDATE services SET description_ru = ? WHERE id = ?", (text, service_id))
    elif field == "stars":
        if not text.isdigit():
            await message.answer("❗ Faqat raqam kiriting:")
            return
        stars_val = int(text)
        supports = 1 if stars_val > 0 else 0
        await db.execute_sql(f"UPDATE services SET stars_price = ?, supports_stars = ? WHERE id = ?", (stars_val, supports, service_id))

    await state.clear()
    s = normalize_service(await db.get_service(service_id))
    await message.answer(
        f"✅ Yangilandi: <b>{s['name']}</b> — {s['price']:,} so'm",
        reply_markup=admin_menu(),
        parse_mode="HTML",
    )


@router.message(EditServiceState.delivery_mode)
async def adm_edit_delivery_mode(message: Message, state: FSMContext):
    if message.text in ADMIN_CANCEL_TEXTS:
        await state.clear()
        await message.answer("Bekor qilindi.", reply_markup=admin_menu())
        return
    ...
    if text == "-":
        await state.clear()
        await message.answer("O'zgartirilmadi.", reply_markup=admin_menu())
        return
    ...
    delivery_mode = delivery_map.get(text)
    if not delivery_mode:
        await message.answer("❌ Tugmalardan birini tanlang:", reply_markup=delivery_mode_keyboard(edit_mode=True))
        return

    await db.execute_sql(
        "UPDATE services SET delivery_mode = ? WHERE id = ?",
        (delivery_mode, service_id)
    )
    await state.clear()
    await message.answer(
        f"✅ Delivery mode yangilandi: <code>{delivery_mode}</code>",
        reply_markup=admin_menu(),
        parse_mode="HTML",
    )
...
async def bonus_manage_start(message: Message, state: FSMContext):
    ...
    data = await db.get_bonus_monitoring()
    text = f"💎 <b>Bonus Monitoringi</b>\n\n"
    text += f"💰 <b>Jami berilgan bonuslar:</b> {data['total_bonus']:,} so'm\n\n"
    
    if data['top_users']:
        text += "👑 <b>Eng ko'p bonusga ega mijozlar (Top 10):</b>\n"
        for i, u in enumerate(data['top_users'], 1):
            name = (u['full_name'] or u['username'] or str(u['id'])).strip()
            if not name or name == ".": name = str(u['id'])
            text += f"{i}. {name} — <b>{u['bonus_balance']:,} so'm</b>\n"
        text += "\n"
        
    if data['recent_txs']:
        text += "🕒 <b>So'nggi bonus amaliyotlari:</b>\n"
        for tx in data['recent_txs']:
            amount_str = f"+{tx['amount']:,}" if tx['amount'] > 0 else f"{tx['amount']:,}"
            dt_str = tx['created_at'][:16]
            text += f"• {dt_str} | ID: <code>{tx['user_id']}</code> | <b>{amount_str}</b> | <i>{tx['reason']}</i>\n"
            
    await message.answer(
        text,
        reply_markup=bonus_monitoring_keyboard(),
        parse_mode="HTML",
    )


@router.callback_query(F.data == "adm_bonus_search")
async def adm_bonus_search(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        return
    await call.answer()
    await state.set_state(BonusManageState.user_id)
    await call.message.answer(
        "🔍 <b>Foydalanuvchi ID kiriting:</b>",
        reply_markup=cancel_keyboard(),
        parse_mode="HTML"
    )
...
@router.message(F.text == "🏆 Referral kampaniyalar")
async def adm_ref_campaigns_list(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    await state.clear()
    campaigns = await db.get_all_referral_campaigns()
    text = "🏆 <b>Referral Kampaniyalar</b>"
    from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
    add_btn = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="➕ Yangi kampaniya", callback_data="adm_ref_camp_add")]
    ])
    if campaigns:
        await message.answer(text, reply_markup=ref_campaigns_admin_keyboard(campaigns), parse_mode="HTML")
    else:
        await message.answer(text + "\n\nHech qanday kampaniya yo'q.", reply_markup=add_btn, parse_mode="HTML")


@router.callback_query(F.data == "adm_ref_camp_list")
async def adm_ref_camp_list_cb(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        return
    await state.clear()
    campaigns = await db.get_all_referral_campaigns()
    ...
    if campaigns:
        await call.message.edit_text("🏆 <b>Referral Kampaniyalar</b>", reply_markup=ref_campaigns_admin_keyboard(campaigns), parse_mode="HTML")
    else:
        await call.message.edit_text("🏆 <b>Referral Kampaniyalar</b>\n\nHech qanday kampaniya yo'q.", reply_markup=add_btn, parse_mode="HTML")
    await call.answer()


@router.callback_query(F.data.startswith("adm_ref_camp:"))
async def adm_ref_camp_detail(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return
    ...
    all_parts = await db.get_all_campaign_participants(campaign_id)
    pending_activations = sum(1 for p in all_parts if p.get("reward_given") and (p.get("activation_status") in ("pending", "ready")))
    status_text = "✅ Faol" if camp["is_active"] else "🔴 Nofaol"
    reward_text = camp.get("reward_description") or (f"{camp['reward_value']} bonus" if camp["reward_type"] == "bonus" else "🎁 Xizmat aktivatsiya")
    deadline_val = camp.get("deadline") or "Yo'q"
    text = (
        f"🏆 <b>{html.escape(camp['name'])}</b>\n\n"
        f"📝 {html.escape(camp.get('description') or '')}\n\n"
        f"👥 Kerakli referal: {camp['required_referrals']}\n"
        f"🎁 Mukofot: {reward_text}\n"
        f"⏳ Muddat: {html.escape(deadline_val)}\n"
        f"📊 Holat: {status_text}\n\n"
        f"👤 Jami Ishtirokchilar: {stats.get('total_participants', 0)}\n"
        f"🏅 Mukofot olganlar: {stats.get('rewarded', 0)}\n"
        f"⏳ Aktivatsiya kutayotganlar: <b>{pending_activations}</b> ta"
    )
    await call.message.edit_text(text, reply_markup=ref_campaign_admin_detail_keyboard(campaign_id, camp["is_active"]), parse_mode="HTML")
    await call.answer()


@router.callback_query(F.data.startswith("adm_ref_camp_toggle:"))
async def adm_ref_camp_toggle(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return
    campaign_id = int(call.data.split(":")[1])
    await db.toggle_referral_campaign(campaign_id)
    camp = await db.get_referral_campaign(campaign_id)
    if not camp:
        await call.answer("Topilmadi", show_alert=True)
        return
...
@router.callback_query(F.data.startswith("adm_ref_camp_delete:"))
async def adm_ref_camp_delete_ask(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return
    campaign_id = int(call.data.split(":")[1])
    from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⚠️ Ha, o'chirish", callback_data=f"adm_ref_camp_del_conf:{campaign_id}")],
        [InlineKeyboardButton(text="🔙 Bekor qilish", callback_data=f"adm_ref_camp:{campaign_id}")]
    ])
    await call.message.edit_text("Haqiqatan ham bu kampaniyani o'chirmoqchimisiz? Barcha ishtirokchilarning ma'lumotlari saqlanib qoladi, lekin kampaniya o'chadi.", reply_markup=kb)

@router.callback_query(F.data.startswith("adm_ref_camp_del_conf:"))
async def adm_ref_camp_delete_confirm(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return
    campaign_id = int(call.data.split(":")[1])
    await db.delete_referral_campaign(campaign_id)
    campaigns = await db.get_all_referral_campaigns()
    from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
    add_btn = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="➕ Yangi kampaniya", callback_data="adm_ref_camp_add")]
...
@router.callback_query(F.data == "adm_ref_camp_add")
async def adm_ref_camp_add_start(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        return
    await state.set_state(CampaignCreateState.name)
    await call.message.answer("Kampaniya nomini kiriting:", reply_markup=cancel_keyboard())
    await call.answer()


@router.message(CampaignCreateState.name)
async def adm_ref_camp_name(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    if message.text in ("❌ Bekor qilish", "Bekor qilish", "Отмена"):
        await state.clear()
        await message.answer("Bekor qilindi.", reply_markup=admin_menu())
        return
    await state.update_data(camp_name=message.text.strip())
    await state.set_state(CampaignCreateState.description)
    await message.answer("Tavsif kiriting (yoki /skip):")


@router.message(CampaignCreateState.description)
async def adm_ref_camp_description(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    if message.text in ("❌ Bekor qilish", "Bekor qilish", "Отмена"):
        await state.clear()
    ...
    await state.update_data(camp_desc=desc)
    await state.set_state(CampaignCreateState.required_referrals)
    await message.answer("Nechta referal kerak? (son kiriting, masalan 5):")


@router.message(CampaignCreateState.required_referrals)
async def adm_ref_camp_required(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    if message.text in ("❌ Bekor qilish", "Bekor qilish", "Отмена"):
        await state.clear()
    ...
    if not message.text.strip().isdigit() or int(message.text.strip()) < 1:
        ...
        return
    await state.update_data(camp_required=int(message.text.strip()))
    await state.set_state(CampaignCreateState.reward_type)
    from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💰 Bonus (pul)", callback_data="camp_rtype:bonus")],
        [InlineKeyboardButton(text="🎁 Xizmat aktivatsiya", callback_data="camp_rtype:service")],
    ])
    await message.answer("Mukofot turini tanlang:", reply_markup=kb)


@router.callback_query(F.data.startswith("camp_rtype:"))
async def adm_ref_camp_rtype(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        return
    rtype = call.data.split(":")[1]
    await state.update_data(camp_reward_type=rtype)
    await state.set_state(CampaignCreateState.reward_value)
    await call.answer()
    if rtype == "bonus":
        await call.message.answer("Bonus miqdorini kiriting (masalan 10000):", reply_markup=cancel_keyboard())
    else:
        await call.message.answer("Xizmat nomini yoki tavsifini kiriting (masalan: ChatGPT Business Plus 1 oy):", reply_markup=cancel_keyboard())


@router.message(CampaignCreateState.reward_value)
async def adm_ref_camp_reward(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    if message.text in ("❌ Bekor qilish", "Bekor qilish", "Отмена"):
        await state.clear()
        ...
        return
    data = await state.get_data()
    reward_type = data.get("camp_reward_type", "bonus")
    if reward_type == "bonus":
        ...
        reward_value = 0  # service type — reward is the service itself
    reward_desc = message.text.strip() if reward_type == "service" else None
    await state.update_data(camp_reward_value=reward_value, camp_reward_desc=reward_desc)
    await state.set_state(CampaignCreateState.deadline)
    await message.answer("Kampaniya uchun tugash muddati (deadline) kiriting (masalan: 1-Maygacha, 2025-yilgacha, yoki 'Yo\\'q' deb yozing):", reply_markup=cancel_keyboard())


@router.message(CampaignCreateState.deadline)
async def adm_ref_camp_deadline(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    if message.text in ("❌ Bekor qilish", "Bekor qilish", "Отмена"):
        await state.clear()
    ...
    deadline = None if message.text.strip().lower() in ("yo'q", "yoq", "net", "no", "skip", "/skip") else message.text.strip()
    data = await state.get_data()
    reward_type = data.get("camp_reward_type", "bonus")

    await db.create_referral_campaign(
        name=data["camp_name"],
        description=data.get("camp_desc", ""),
        required_referrals=data["camp_required"],
        reward_type=reward_type,
        reward_value=data.get("camp_reward_value", 0),
        reward_description=data.get("camp_reward_desc"),
        deadline=deadline
    )
    await state.clear()
    campaigns = await db.get_all_referral_campaigns()
    await message.answer(
        f"✅ Kampaniya <b>{html.escape(data['camp_name'])}</b> yaratildi!",
        reply_markup=ref_campaigns_admin_keyboard(campaigns),
        parse_mode="HTML",
    )


@router.callback_query(F.data.startswith("adm_ref_camp_stats:"))
async def adm_ref_camp_stats(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return
    campaign_id = int(call.data.split(":")[1])
    camp = await db.get_referral_campaign(campaign_id)
    ...
    stats = await db.get_campaign_stats(campaign_id)
    text = (
        f"📊 <b>{html.escape(camp['name'])} — Statistika</b>\n\n"
        f"👤 Jami ishtirokchilar: {stats.get('total_participants', 0)}\n"
        f"🏅 Mukofot olganlar: {stats.get('rewarded', 0)}\n"
    ...
    back_kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔙 Orqaga", callback_data=f"adm_ref_camp:{campaign_id}")]
    ])
...
@router.callback_query(F.data.startswith("adm_ref_camp_parts:"))
async def adm_ref_camp_participants(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return
    campaign_id = int(call.data.split(":")[1])
    camp = await db.get_referral_campaign(campaign_id)
    if not camp:
        await call.answer("Topilmadi", show_alert=True)
        return
    completed = await db.get_all_campaign_participants(campaign_id)
    if not completed:
        from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
        back_kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 Orqaga", callback_data=f"adm_ref_camp:{campaign_id}")]
        ...
        await call.message.edit_text(
            f"👥 <b>{html.escape(camp['name'])} — Ishtirokchilar</b>\n\nIshtirokchilar yo'q.",
    ...
    buttons = []
    for p in completed:
        username = f"@{p['username']}" if p.get("username") else (p.get("full_name") or f"ID {p['user_id']}")
        # Different emoji based on completion
        if p.get("reward_given"):
            act_st = p.get("activation_status") or "pending"
            emoji = {"activated": "✅", "ready": "🟢", "pending": "⏳"}.get(act_st, "⏳")
        else:
            emoji = "🏃"  # In progress
        email = p.get("customer_email") or f"{p.get('current_referrals', 0)}/{camp['required_referrals']} ref"
        buttons.append([InlineKeyboardButton(
            text=f"{emoji} {username} | {email[:20]}",
            callback_data=f"adm_camp_part:{campaign_id}:{p['user_id']}"
        )])
    buttons.append([InlineKeyboardButton(text="🔙 Orqaga", callback_data=f"adm_ref_camp:{campaign_id}")])
    await call.message.edit_text(
        f"👥 <b>{html.escape(camp['name'])} — Barcha ishtirokchilar</b>",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons),
        parse_mode="HTML",
    )
    await call.answer()
...
@router.callback_query(F.data.startswith("adm_camp_part:"))
async def adm_ref_camp_participant_detail(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return
    parts = call.data.split(":")
    ...
    user_id = int(parts[2])
    camp = await db.get_referral_campaign(campaign_id)
    participant = await db.get_campaign_participant(campaign_id, user_id)
    user = await db.get_user(user_id)
    if not camp or not participant:
        await call.answer("Topilmadi", show_alert=True)
        return
    act_st = participant.get("activation_status") or "pending"
    act_emoji = {"activated": "✅", "ready": "🟢", "pending": "⏳"}.get(act_st, "⏳")
    username = f"@{user['username']}" if user and user.get("username") else "nomalum"
    cur = int(participant.get("current_referrals") or 0)
    req = int(camp["required_referrals"])
    bar_filled = min(10, round(cur / max(req, 1) * 10)) if req else 0
    bar = "🟩" * bar_filled + "⬜" * (10 - bar_filled)
    text = (
        f"👤 <b>Ishtirokchi</b>\n\n"
        f"User: {username} (<code>{user_id}</code>)\n"
        f"Kampaniya: {html.escape(camp['name'])}\n\n"
        f"📊 Progress: {cur}/{req}\n"
        f"{bar}\n\n"
        f"📧 Email: <code>{participant.get('customer_email') or '-'}</code>\n"
        f"{act_emoji} Holat: <b>{act_st}</b>\n"
        f"✅ Activated at: {participant.get('activated_at') or '-'}"
    )
    await call.message.edit_text(
        text,
        reply_markup=ref_campaign_participant_keyboard(campaign_id, user_id, act_st),
        parse_mode="HTML",
    )
    await call.answer()


@router.callback_query(F.data.startswith("adm_camp_activate:"))
async def adm_ref_camp_activate(call: CallbackQuery, bot: Bot):
    if not is_admin(call.from_user.id):
        return
    parts = call.data.split(":")
    campaign_id = int(parts[1])
    user_id = int(parts[2])
    await db.mark_campaign_participant_activated(campaign_id, user_id)
    camp = await db.get_referral_campaign(campaign_id)
    ...
    try:
        if user_lang == "uz":
            await bot.send_message(
                user_id,
                f"✅ <b>{html.escape(camp_name)}</b> kampaniyasi mukofotingiz aktivlashtirildi!\n\n"
                f"Admin tez orada xizmatni yetkazadi.",
                parse_mode="HTML",
            )
        else:
            await bot.send_message(
                user_id,
                f"✅ Награда кампании <b>{html.escape(camp_name)}</b> активирована!\n\n"
                f"Админ скоро доставит услугу.",
                parse_mode="HTML",
            )
    except Exception:
        pass
    ...
    participant = await db.get_campaign_participant(campaign_id, user_id)
    act_st = participant.get("activation_status") or "activated"
    act_emoji = "✅"
    username = f"@{user['username']}" if user and user.get("username") else "nomalum"
    text = (
        f"👤 <b>Ishtirokchi</b>\n\n"
        f"User: {username} (<code>{user_id}</code>)\n"
        f"Kampaniya: {html.escape(camp_name)}\n\n"
        f"📧 Email: <code>{participant.get('customer_email') or '-'}</code>\n"
        f"{act_emoji} Holat: <b>{act_st}</b>\n"
        f"✅ Activated at: {participant.get('activated_at') or '-'}"
    )
...
@router.callback_query(F.data.startswith("adm_ref_camp_edit:"))
async def adm_ref_camp_edit_start(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        return
    campaign_id = int(call.data.split(":")[1])
    camp = await db.get_referral_campaign(campaign_id)
    ...
    from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📝 Nom o'zgartirish", callback_data=f"camp_edit_field:{campaign_id}:name")],
        [InlineKeyboardButton(text="📄 Tavsif o'zgartirish", callback_data=f"camp_edit_field:{campaign_id}:description")],
        [InlineKeyboardButton(text="👥 Referal soni o'zgartirish", callback_data=f"camp_edit_field:{campaign_id}:required_referrals")],
        [InlineKeyboardButton(text="⏳ Muddat (deadline)", callback_data=f"camp_edit_field:{campaign_id}:deadline")],
        [InlineKeyboardButton(text="🔙 Orqaga", callback_data=f"adm_ref_camp:{campaign_id}")],
    ])
    await call.message.edit_text(
        f"✏️ <b>{html.escape(camp['name'])}</b> — qaysi maydonni o'zgartirasiz?",
        reply_markup=kb, parse_mode="HTML"
    )
    await call.answer()


@router.callback_query(F.data.startswith("camp_edit_field:"))
async def adm_ref_camp_edit_field(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        return
    parts = call.data.split(":")
    campaign_id = int(parts[1])
    field = parts[2]
    await state.update_data(camp_edit_id=campaign_id, camp_edit_field=field)
    await state.set_state(CampaignEditState.value)
    labels = {"name": "Yangi nom", "description": "Yangi tavsif", "required_referrals": "Yangi referal soni (raqam)", "deadline": "Yangi muddat (masalan: 1-Maygacha. O'chirish uchun 'yo\\'q' yozing)"}
    await call.message.answer(f"{labels.get(field, 'Yangi qiymat')} kiriting:", reply_markup=cancel_keyboard())
    await call.answer()


@router.message(CampaignEditState.value)
async def adm_ref_camp_edit_save(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    if message.text in ("❌ Bekor qilish", "Bekor qilish", "Отмена"):
        await state.clear()
        ...
        return
    data = await state.get_data()
    campaign_id = data.get("camp_edit_id")
    field = data.get("camp_edit_field")
    value = message.text.strip()
    if field == "required_referrals":
        if not value.isdigit() or int(value) < 1:
            await message.answer("Iltimos, musbat son kiriting:")
            return
        value = int(value)
    elif field == "deadline":
        if value.lower() in ("yo'q", "yoq", "net", "no", "skip", "/skip"):
            value = None
    import aiosqlite
    from config import DB_PATH
    async with aiosqlite.connect(DB_PATH) as conn:
        await conn.execute(f"UPDATE referral_campaigns SET {field}=? WHERE id=?", (value, campaign_id))
        await conn.commit()
    await state.clear()
    camp = await db.get_referral_campaign(campaign_id)
    if camp:
        stats = await db.get_campaign_stats(campaign_id)
        status_text = "✅ Faol" if camp["is_active"] else "🔴 Nofaol"
        reward_text = f"{camp['reward_value']} bonus" if camp["reward_type"] == "bonus" else "🎁 Xizmat"
        deadline_val = camp.get("deadline") or "Yo'q"
        text = (
            f"✅ O'zgartirildi!\n\n"
            f"🏆 <b>{html.escape(camp['name'])}</b>\n\n"
            ...
            f"⏳ Muddat: {html.escape(deadline_val)}\n"
            f"📊 Holat: {status_text}\n\n"
            f"👤 Ishtirokchilar: {stats.get('total_participants', 0)}\n"
            f"🏅 Mukofot olganlar: {stats.get('rewarded', 0)}"
        )
        await message.answer(text, reply_markup=ref_campaign_admin_detail_keyboard(campaign_id, camp["is_active"]), parse_mode="HTML")
    else:
        await message.answer("Kampaniya topilmadi.", reply_markup=admin_menu())
