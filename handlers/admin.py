...
async def _build_referral_order_detail(order_id: int) -> str:
    ...
    return (
        ...
        f"Mode: {service.get('delivery_mode', 'manual') if service else 'manual'}\n"
        f"Email: <code>{order.get('customer_email') or '-'}</code>\n"
        f"Progress: {order.get('current_referrals', 0)} / {order.get('required_referrals', 0)}\n"
        f"Deadline: {deadline}\n"
        f"Referral status: {order.get('referral_status')}\n"
        f"Activation status: {order.get('activation_status')}\n"
        f"Activation ready at: {order.get('activation_ready_at') or '-'}\n"
...
async def referral_activation_view(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return
    _, _, order_id_raw, current_filter = call.data.split(":")
    order_id = int(order_id_raw)
    await call.answer()
    text = await _build_referral_order_detail(order_id)
    await call.message.edit_text(
        text,
        reply_markup=referral_order_detail_keyboard(order_id, current_filter),
        parse_mode="HTML",
    )
...
async def referral_activation_deadline(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return
    _, _, order_id_raw, days_raw, current_filter = call.data.split(":")
    order_id = int(order_id_raw)
    days = int(days_raw)
...
async def confirm_order(call: CallbackQuery, bot: Bot):
    ...
    if service and service.get("delivery_mode") == "referral_activation":
        ...
        await call.message.reply(
            ...
            reply_markup=referral_order_detail_keyboard(order_id, "pending"),
            parse_mode="HTML",
        )
        return
...
def _build_keyguide_message(key_text: str, delivery_content: str) -> str:
    ...
    parts.extend([
        "",
        "? Savol tug?ilsa, shu chat orqali bizga yozishingiz mumkin."
    ])
...
async def adm_add_requires_email(message: Message, state: FSMContext):
    ...
    await message.answer("? Referral deadline days kiriting (0, 7, 14, 30):", reply_markup=cancel_keyboard())
...
async def adm_edit_requires_email(message: Message, state: FSMContext):
    ...
    await message.answer("? Deadline days ('-' = o'zgartirmaslik, 0/7/14/30):", reply_markup=cancel_keyboard())
